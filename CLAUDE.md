...
| Action | `4cloudguru/shared-workflows/.github/actions/release-pr-closing-keywords@6cc136d063217a077fc5183595adf26b21626a82` (v1.20.2) |
...
| `output-variables.ts`               | The output-variable boundary — length/printable-ASCII sanitization and the Packer manifest read that sets `artifactId`/`manifestFilePath` — extracted from the base handler (#113). `OutputBoundaryClassL0` scans this file structurally on Windows, where the behavioural test cannot run |
| `environment-variable-policy.ts`    | The `environmentVariables` passthrough policy — rejects names that SELECT a cloud identity, warns on names this task manages, masks secret-shaped values — extracted from the base handler (#113). Runs before `handleProvider()`, which is why a name that selects an identity fails the task rather than being warned about |
