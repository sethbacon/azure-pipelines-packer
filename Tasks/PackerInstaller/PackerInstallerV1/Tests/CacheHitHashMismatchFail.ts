...
// #136: a cache hit whose recorded sidecar hash does NOT match the cached binary's
// actual (mocked) hash must fail the task -- this is the local-tamper/corruption
// detection the cache-hit re-verification exists to catch.
...
tr.registerMock('fs', {
  ...
  readFileSync: (filePath: string, _encoding?: string) => {
    if (filePath === '/tmp/packer-cached/packer.sha256') {
      // Recorded (stale/tampered-with) hash -- does not match the binary below.
      return 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
    }
    return Buffer.from('current-binary-content');
  },
  writeFileSync: (_path: string, _data: string) => { throw new Error('Should not write a new hash when one already exists'); }
});
...
tr.registerMock('crypto', {
  ...
  createHash: (_algorithm: string) => ({
    update: (_data: unknown) => ({
      // Actual hash of the (mocked) current binary content -- deliberately
      // different from the recorded sidecar hash above.
      digest: (_encoding: string) => 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'
    })
  })
});
