...
const SITE_ROWS: SiteRow[] = [
    {
        ...
        why: 'registry-advertised download_url: host chosen by the registry, so authorized on the initial URL and every hop (#161)',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
        fn: 'downloadZipFromMirror', sink: 'downloadToFile', verdict: 'AUTHORIZED',
        why: 'mirror binary download: the reported site of #161, now authorized per hop with DNS resolution (#191)',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
        fn: 'downloadZipFromMirror', sink: 'fetchTextAllow404', verdict: 'AUTHORIZED',
        why: 'mirror SHA256SUMS fetch shares the authorized function and fetchWithTimeout\'s same-origin redirect policy',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
        fn: 'downloadZipFromRegistry', sink: 'fetchJson', verdict: 'AUTHORIZED',
        why: 'registry info endpoint, in the authorized function',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
        fn: 'resolveVersionFromRegistry', sink: 'fetchJson', verdict: 'EXEMPT-OPERATOR-DECLARED',
        why: 'destination is the registryUrl the pipeline author typed, HTTPS-pinned, and fetchWithTimeout refuses any off-host redirect — no third party chooses this host',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
        fn: 'resolveVersionFromHashiCorp', sink: 'fetchJson', verdict: 'EXEMPT-CONSTANT-HOST',
        why: 'checkpoint-api.hashicorp.com is a compile-time constant host',
    },
    {
        file: 'Tasks/PackerInstaller/PackerInstallerV1/src/packer-installer.ts',
    ...
    },
    {
        ...
        why: 'releases.hashicorp.com SHA256SUMS, constant host',
    },
];
...
describe('egress authorization (class test #161/#188/#191/#200/#201)', function () {
            ...
            it(`${row.rejected ? 'refuses' : 'follows'} a ${row.what}`, async () => {
                ...
                try {
                    const run = downloadToFile(
                        'https://artifacts.example.com/packer.zip',
                        destination,
                        5000,
        ...
        it('refuses a hop that an explicit allowlist does not cover', async () => {
            ...
            try {
                await assert.rejects(
                    downloadToFile('https://artifacts.example.com/packer.zip', destination, 5000,
    ...
    describe('E. every guard message resolves through task.json (#201)', () => {
        ...
        // guard renders as its raw key name. Assert the two SSRF rejections (and
        // every other loc key) are declared where task-lib actually reads them.
        const taskJson = JSON.parse(fs.readFileSync(path.join(__dirname, '../task.json'), 'utf8')) as { messages: Record<string, string> };
        const resjson = JSON.parse(fs.readFileSync(path.join(__dirname, '../Strings/resources.resjson/en-US/resources.resjson'), 'utf8')) as Record<string, string>;
        const source = fs.readFileSync(path.join(__dirname, '../src/packer-installer.ts'), 'utf8');
        ...
        for (const key of GUARD_KEYS) {
            it(`${key} is declared in task.json messages`, () => {
                ...
                assert.strictEqual(resjson[`loc.messages.${key}`], taskJson.messages[key], `${key} text differs between task.json and the resjson`);
            });
        }

        it('every tasks.loc() key used by the installer is declared in task.json', () => {
            const used = [...source.matchAll(/\bloc\(\s*['"`]([A-Za-z0-9_]+)['"`]/g)].map(m => m[1]);
