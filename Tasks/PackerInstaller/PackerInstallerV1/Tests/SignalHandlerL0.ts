 ...
 * unhandledRejection registration (#1113). This task writes no sensitive temp
 * file, so cleanup() is a deliberate no-op -- but the handler is still
 * registered so a cancelled run dies promptly (re-raising the signal with its
 * default disposition) instead of lingering, and an unawaited rejection
 * anywhere in a helper no longer falls through to Node's default handling
 * with no tasks.setResult call and no deterministic exit code.
 *
 ...
 */
describe('index.ts SIGTERM/SIGINT registration -- re-raise after (no-op) cleanup (azure-pipelines-terraform#1113, suite-scope residual)', function () {
