 ...
 *
 * Run in BOTH extensions: the sibling azure-pipelines-terraform repo shipped the
 * same #197 code change but left its `task.json` defaults in place, which made
 * the derivation dead code there. This copy pins the property here.
 *
 * #197 replaced the fixed AWS role session names (`AzureDevOps-Packer`) with a per-run derivation, because a constant
 * collapses CloudTrail attribution across every federated run of every pipeline
 * in every organization using this extension.
 *
 * The trap the sibling repo fell into: changing the code fallback while leaving
 * `"defaultValue": "AzureDevOps-Packer"` in `task.json`. The ADO agent supplies a
 * declared default when the pipeline author leaves the input blank, so
 * `tasks.getInput('awsSessionName')` would return the constant on every real run,
 * `resolveRoleSessionName()` would take its explicit-input branch, and the
 * derivation would be dead code in production. The mock-runner tests cannot see
 * that: the mock runner only supplies inputs a test sets with `tr.setInput()`, so
 * it never applies a manifest default. This table is what keeps the default clear.
 *
...
const RETIRED_CONSTANTS = ['AzureDevOps-Packer'];

/** (input, prefix) pairs as wired in aws-packer-command-handler.ts. */
const DERIVATION_ROWS: Array<{ input: string; prefix: string }> = [
    { input: 'awsSessionName', prefix: 'ado-packer' },
];
