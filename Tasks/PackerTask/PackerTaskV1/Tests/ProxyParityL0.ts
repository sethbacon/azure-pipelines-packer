 ...
 *                     enumerates in this repo, with its verdict. This
 *                     repository no longer carries that signature: it is the
 *                     shared `check-proxy-parity` composite action in
 *                     4cloudguru/shared-workflows, which this job `uses:` by
 *                     SHA before the suite runs, so the bytes asserted below
 *                     are the bytes the pin names.
 *
