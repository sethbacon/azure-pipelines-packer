...
export abstract class BasePackerCommandHandler {
    ...
    /**
     * Applies the operator-supplied `environmentVariables` passthrough under the
     * policy in environment-variable-policy.ts (#113).
     *
     * Stays a protected METHOD rather than becoming a direct call: PassthroughEnv-
     * ClassificationL0 invokes it reflectively as
     * `(handler as any).applyEnvironmentVariables()`, and PrototypeSafeLookupClassL0's
     * ProbeHandler OVERRIDES it to a no-op. Both break if it stops being an
     * overridable member.
     */
    protected applyEnvironmentVariables(): void {
        applyPassthroughEnvironmentVariables();
    }
