...
export class TokenGenerator {
    ...
    public async generate(serviceConnectionID: string): Promise<string> {
        ...
        // header; assert https:// for parity with the installer's http-client
        // transport hardening before that token is ever sent anywhere (#107).
        let scheme: string;
        try {
        ...
        for (let attempt = 1; attempt <= TokenGenerator.MAX_RETRIES; attempt++) {
            try {
                ...
                // transient, matching the installer http-client's classifier. A
                // deterministic 4xx (bad/expired access token, misconfigured service
                // connection) is non-retryable and skips the remaining attempts and
                // their backoff delay.
    ...
    private async fetchToken(url: string, oidcRequestUri: string): Promise<string> {
        ...
        if (!oidcObject?.oidcToken) {
            throw new Error(tasks.loc("Error_FederatedTokenAcquisitionFailed"));
        }
