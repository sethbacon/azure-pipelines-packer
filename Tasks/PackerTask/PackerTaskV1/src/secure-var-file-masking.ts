// @shared-module: copied from azure-pipelines-terraform (Tasks/TerraformTask/TerraformTaskV5/src/secure-var-file-masking.ts)
// @shared-module-policy: Secure var-file value EXTRACTION is the security-relevant half and
//   must stay in parity. Apply extractor fixes to BOTH repositories' copies. Enforced by
//   scripts/check-shared-modules.js.
// @shared-module-status: IN-SYNC — brought to parity on #1105. This file was copied from the
//   sibling and then drifted UNGATED: it was registered in neither this repo's PROVENANCE list
//   nor terraform's byte-identical FAMILIES list, so an identical extractor defect survived in
//   BOTH copies past every gate. The registration is the durable fix; the extractor repair is
//   the instance. The two copies are deliberately NOT byte-identical below the extractor: this
//   one calls tasks.setSecret() directly where the sibling routes through
//   EnvironmentVariableHelper.registerSecret(), which this extension does not carry.
