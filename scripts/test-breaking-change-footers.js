...
// in .github/workflows/pr-checks.yml (#274).
//
...
function runGuard(commits, stubDir = binDir) {
    ...
    const result = spawnSync('bash', [scriptPath], {
        ...
        env: {
            ...
            GH_TOKEN: 'stub',
            PR_NUMBER: '123',
            REPO: 'sethbacon/azure-pipelines-packer',
...
const FOOTER = 'BREAKING CHANGE: the mirror input is no longer optional';
...
try {
    ...
    expectPass('clean-single', [`feat: rework the installer download path\n\n${FOOTER}`], ['declarations in this PR: 1']);
    ...
    expectPass('footer-plus-bang-one-commit', [`feat!: rework the installer download path\n\n${FOOTER}`], ['declarations in this PR: 1']);
    ...
    expectPass(
        ...
        ['feat: rework the installer download path\n\nBREAKING-CHANGE: the input is no longer optional'],
        ['declarations in this PR: 1'],
    );

    console.log('\nmutations — the squash losing a declaration (#274):');
    // THE case: registry-backend v4.0.0 published two breaking changes and
    // documented one.
    expectRejection(
        'two-footers',
        [`feat: drop the legacy template input\n\n${FOOTER}`, 'feat: require a service connection\n\nBREAKING CHANGE: the inline credentials input is gone'],
        ['declares 2 breaking changes', 'the squash keeps only the first'],
        ['**2** breaking changes', 'abc000', 'abc001'],
    );
    expectRejection(
        'two-bang-headers',
        ['feat!: drop the legacy template input', 'fix(auth)!: require a service connection'],
        ['declares 2 breaking changes'],
        ['drop the legacy template input', 'require a service connection'],
    );
